...
void blendfunc_buf(Color* src, Color* dest, float amount, size_t n)
{
	for(size_t i = 0; i < n; i++)
	{
		dest[i] = func(src[i], dest[i], amount);
	}
}

void
Color::blend_buf(Color *src, Color *dest, float amount, size_t n, const Color::BlendMethod type)
{
	if(fabsf(amount)<=COLOR_EPSILON) return;

	assert(type<BLEND_END);

	typedef void(*buf_func)(Color*, Color*, float, size_t);

	const static buf_func vtable[BLEND_END]=
	{
		blendfunc_buf<blendfunc_COMPOSITE>,	// 0
		blendfunc_buf<blendfunc_STRAIGHT>,
		blendfunc_buf<blendfunc_BRIGHTEN>,
		blendfunc_buf<blendfunc_DARKEN>,
		blendfunc_buf<blendfunc_ADD>,
		blendfunc_buf<blendfunc_SUBTRACT>,		// 5
		blendfunc_buf<blendfunc_MULTIPLY>,
		blendfunc_buf<blendfunc_DIVIDE>,
		blendfunc_buf<blendfunc_COLOR>,
		blendfunc_buf<blendfunc_HUE>,
		blendfunc_buf<blendfunc_SATURATION>,	// 10
		blendfunc_buf<blendfunc_LUMINANCE>,
		blendfunc_buf<blendfunc_BEHIND>,
		blendfunc_buf<blendfunc_ONTO>,
		blendfunc_buf<blendfunc_ALPHA_BRIGHTEN>,
		blendfunc_buf<blendfunc_ALPHA_DARKEN>,	// 15
		blendfunc_buf<blendfunc_SCREEN>,
		blendfunc_buf<blendfunc_HARD_LIGHT>,
		blendfunc_buf<blendfunc_DIFFERENCE>,
		blendfunc_buf<blendfunc_ALPHA_OVER>,
		blendfunc_buf<blendfunc_OVERLAY>,		// 20
		blendfunc_buf<blendfunc_STRAIGHT_ONTO>,
		blendfunc_buf<blendfunc_ADD_COMPOSITE>,
		blendfunc_buf<blendfunc_ALPHA>,
	};

	vtable[type](src, dest, amount, n);
}

void
Color::blend_buf(Color *srcA, Color srcB, Color *dest, float amount, size_t n, BlendMethod type)
{
